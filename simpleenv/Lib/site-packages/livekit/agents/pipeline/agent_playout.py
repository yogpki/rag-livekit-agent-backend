...
# 从当前文件路径逐级向上查找项目根目录
def find_project_root(current_dir, target_folder="rag-livekit-agent-backend"):
    while True:
        # 检查当前目录是否包含标志文件夹（即项目根目录）
        if os.path.basename(current_dir) == target_folder:
            return current_dir
        parent_dir = os.path.dirname(current_dir)
        if parent_dir == current_dir:  # 已经到达文件系统的根目录
            raise FileNotFoundError(f"Could not find the project root folder: {target_folder}")
...
a2f_path = os.path.join(project_root, "a2f")

# 添加 a2f 文件夹到 sys.path
sys.path.append(a2f_path)
